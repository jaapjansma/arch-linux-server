...
mysql -e "UPDATE mysql.user SET Password = PASSWORD('$(random_passwd_root)') WHERE User = 'root'"
...
/usr/local/bin/arch-linux-server/scripts/send-email-from-dir.py --directory=/tmp/mails
rm -rf /tmp/mails
